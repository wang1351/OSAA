...
def data_generator(data_path, domain_id, dataset_configs, hparams):
    ...
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size,
                                              shuffle=True, drop_last=dataset_configs.drop_last, num_workers=8)
    return train_loader, test_loader

def data_inter_generator(data_path, domain_id, dataset_configs, hparams):
    # loading path
    dataset = torch.load(os.path.join(data_path, "inter_" + domain_id + ".pt"))
    

    # Loading datasets
    dataset = Load_Dataset(dataset, dataset_configs.normalize)
